...
def getSimilarityCategory(cat_a,cat_b):
	...
	if len(path_a)==0 or len(path_b)==0:
		return 0
	mini_path_length = sys.maxint
	a_min = 0.0
	b_min = 0.0
	for a in path_a:
		for b in path_b:
			depth_lca = get_depth_lca(a,b)
			curr_path_length = len(a)+len(b)-2*depth_lca+1
			if curr_path_length < mini_path_length:
				mini_path_length = curr_path_length
				a_min = a
				b_min = b
	return mini_path_length
...
def getSimilarityCategories(cats_a,cats_b):
	...
	for a in cats_a:
		...
		category_list.append(min_b)
	if len(category_list) == 0:
		return 0.0
	# return round(sum(category_list)/(1.0*len(category_list)),2)
	return min(category_list)*1.0
...
part = int(sys.argv[1])
CN_simi_analysis = open("CN_simi_PL_analysis_min_"+str(part),"w")
