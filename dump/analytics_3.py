...
filter_over_05 = open("over_05_3","w")
dirty = open("over_05_dirty_3","w")
hop_3_cat = open("3_hop_cat","r").read().splitlines()
hop_3_cat = set(hop_3_cat)
error_f = open("error_f_3","w")
...
for x in xrange(0,len(columns_nz_6by3)):
	...
	if len(cats_wiki) == 0 or cats_wiki is None:
		error_f.write(wiki+"\n")
		continue
	if len(hop_3_cat.intersection(set(cats_wiki))) > 0:
