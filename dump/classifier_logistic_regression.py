...
wiki_As,train_yes,train_no = getTrainingTestingData("features5.csv",len(X_yes),scale = 1)
...
logistic_regression_model = LogisticRegression(C=1, penalty='l1', tol=0.01)
...
for key in keys:
	...
	c = f1_score(y_test, list(y_pred),pos_label='Y', average='binary')
	pre_mi.append(a)
	rec_mi.append(b)
	f1s.append(c+0.01)
