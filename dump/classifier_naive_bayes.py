...
def getTrainingTestingData(filename,train_size,scale = 1):
	...
	for key in keys:
		if len(train_yes) < training_size and len(train_no) < training_size:
			...
			train_no = train_no + wiki_dict[key]['N'][0:len(wiki_dict[key]['Y'])]
			del wiki_dict[key]
...
# Linear Kernel
nb_gnb_clf = GaussianNB()
nb_gnb_clf.fit(X_train, y_train)
...
for key in keys:
	...
	X_test = wiki_As[key]['Y']+wiki_As[key]['N'][0:number_yes]
	y_test = ['Y']*number_yes + ['N']*min(number_yes,len(wiki_As[key]['N']))
	predicted= nb_gnb_clf.predict(X_test)
