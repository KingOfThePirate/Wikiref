...
def my_precision_score(ground_truth,predicted):
	num = 0
	den = 0
	# print "GT",ground_truth
	# print "Pre",predicted
	for i in range(len(ground_truth)):
		if predicted[i] == 'Y' and ground_truth[i] == predicted[i]:
			num += 1
		if predicted[i] == 'Y':
			den += 1
	try:
		precision = round((num*1.0)/(den*1.0),2)
		return precision
...
def getTrainingTestingData(filename,train_size,scale = 1):
	...
	for key in keys:
		if len(train_yes) < training_size :
			...
			break
	train_size_no = min(len(train_no),int(1.5*training_size))
	# Random-ness
	# shuffle(train_no)
	train_no = train_no[0:train_size_no]
...
wiki_As,train_yes,train_no = getTrainingTestingData("features2.csv",len(X_yes),scale = 2)
...
for key in keys:
	...
	a = my_precision_score(y_test, list(predicted))
