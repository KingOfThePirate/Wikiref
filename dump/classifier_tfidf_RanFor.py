...
def getRefContexttitle(B,ref_dict):
	B = B.replace(" ","_")
	if B[-5:] != ".wiki":
		B = B+".wiki"
...
def getBsRef(A,Bs):
	...
	for B in Bs:
		...
		ref_context = getRefContexttitle(B,ref_dict_B)
...
for key in keys:
	...
	# print "ref_by_b",ref_by_b
	# print "ref_overlap_A",ref_overlap_A
	# print "ref_dict_A",ref_dict_A
	if len(ref_by_b) < 1:
		pre.append(1.0)
	else:	
...
save_pickle(pre,"pre_"+sys.argv[2]+"_"+sys.argv[1])
save_pickle(rec,"rec_"+sys.argv[2]+"_"+sys.argv[1])