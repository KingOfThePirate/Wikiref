...
def getBsRef(A,Bs):
	...
	for B in Bs:
		...
		for key in ref_dict_B:
			simi_pcn = getSimilarity(sentence_MC_vector_embeddings[A][B][key],sentence_MC_vector_embeddings[A][B]['pcn'])
...
sentence_MC_vector_embeddings = load_pickle("sentence_MC_vector_embeddings")
...
for key in keys:
	...
	# print A
	print "Average Precision & Recall k = 1: ",round(sum(pre_k1)/len(pre_k1),2),round(sum(rec_k1)/len(rec_k1),2)
	print "Average Precision & Recall k = 2: ",round(sum(pre_k2)/len(pre_k2),2),round(sum(rec_k2)/len(rec_k2),2)
	print "Average Precision & Recall  k = 3: ",round(sum(pre_k3)/len(pre_k3),2),round(sum(rec_k3)/len(rec_k3),2)
	print "Average Precision & Recall  k = 4: ",round(sum(pre_k4)/len(pre_k4),2),round(sum(rec_k4)/len(rec_k4),2)
	print "Average Precision & Recall  k = 5: ",round(sum(pre_k5)/len(pre_k5),2),round(sum(rec_k5)/len(rec_k5),2)
	print "Average Precision & Recall  k = 10: ",round(sum(pre_k10)/len(pre_k10),2),round(sum(rec_k10)/len(rec_k10),2)
