...
import csv,math,sys,gensim,pickle,re,jellyfish,bisect,torch
...
def build_train_data(A,Bs,fp,qid):
	ref_dict_A = getRef(A)
		...
		for key in ref_context:
			...
			try:
				...
				try:
					...
					print "train key error",A,B
					continue
			try:
				simi_vec_title_pcn = getSimilarityBtwVec(ref_context_title[key],pcn)
			except KeyError:
				try:
					simi_vec_title_pcn = getSimilarityBtwVec(ref_context_title[key],pcn)
				except KeyError:
			...
			if key in similar_ref_AB:
				fp.write("1 qid:"+str(qid)+" "+"1:"+str(simi_tfidf_pcn)+" "+"2:"+str(simi_vec_pcn)+" 3:"+str(simi_tfidf_title_pcn)+ " 4:"+str(simi_vec_title_pcn)  +" \n")
			else:
				fp.write("-1 qid:"+str(qid)+" "+"1:"+str(simi_tfidf_pcn)+" "+"2:"+str(simi_vec_pcn)+" 3:"+str(simi_tfidf_title_pcn)+ " 4:"+str(simi_vec_title_pcn)  +" \n")


dataset_path = "../../dataset/ComputerScience/"
numberOfYes = getNumberOfYes("../features.csv")
training_testing_variation = 0.3
wiki_As,train_yes,train_no = getTrainingTestingData("../features.csv",numberOfYes,scale = 1)
train_data_SVMRank = open("train_data_SVMRank_4F.dat","w")
...
for key in keys:
	...
	build_train_data(key,predicted_yes,train_data_SVMRank,count)
	count = count + 1

train_data_SVMRank.close()